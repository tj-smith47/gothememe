...
func (g *Generator) Generate() error {
	...
	// Scan and include custom themes
	customThemes, err := ScanCustomThemes(g.outputDir)
	if err != nil {
		return fmt.Errorf("scanning custom themes: %w", err)
	}
	for _, ct := range customThemes {
		// Add custom theme to seen maps to prevent duplicates
		seenIDs[ct.ID] = 1
		seenVarNames[ct.VarName] = 1

		themeInfos = append(themeInfos, themeInfo{
			ID:           ct.ID,
			VarName:      ct.VarName,
			InstanceName: "theme" + ct.VarName + "Instance",
			DisplayName:  ct.DisplayName,
			FileName:     ct.FileName,
			IsDark:       ct.IsDark,
		})
		fmt.Printf("Included custom theme: %s (%s)\n", ct.DisplayName, ct.ID)
	}
...
func (g *Generator) GenerateMarkdown(outputPath string) error {
	...
	// Scan and include custom themes from themes directory
	customThemes, err := ScanCustomThemes(themesDir)
	if err != nil {
		return fmt.Errorf("scanning custom themes: %w", err)
	}
	for _, ct := range customThemes {
		// Add custom theme to seen maps
		seenIDs[ct.ID] = 1
		seenVarNames[ct.VarName] = 1

		// Generate SVG preview for custom theme
		svgPath := fmt.Sprintf("themes/%s.svg", ct.ID)
		svgFilePath := filepath.Join(filepath.Dir(outputPath), svgPath)

		// Check if SVG already exists, if not generate a placeholder
		if _, err := os.Stat(svgFilePath); os.IsNotExist(err) {
			// Generate a simple SVG preview for custom themes
			// This is a placeholder - custom themes should provide their own SVGs
			svg := generateCustomThemeSVG(ct.DisplayName)
			if err := os.WriteFile(svgFilePath, []byte(svg), 0o644); err != nil { //nolint:gosec // G306: 0644 is intentional for generated SVG
				fmt.Printf("Warning: failed to generate SVG for %s: %v\n", ct.DisplayName, err)
			}
		}
...
// generateCustomThemeSVG creates a placeholder SVG for custom themes.
func generateCustomThemeSVG(displayName string) string {
	return fmt.Sprintf(`<svg xmlns="http://www.w3.org/2000/svg" width="400" height="80" viewBox="0 0 400 80">
  <title>%s color theme preview</title>
  <rect x="0" y="0" width="400" height="80" fill="#282a36"/>
  <text x="200" y="45" font-family="Arial, sans-serif" font-size="16" fill="#f8f8f2" text-anchor="middle">%s</text>
  <rect x="0" y="0" width="400" height="80" fill="none" stroke="rgba(0,0,0,0.2)" stroke-width="1"/>
